...
OUTPUT_DIR = PROJECT_ROOT / "inference" / "benchmarking" / DATASET_TAG / "cellpose_sam"
INPUT_MANIFEST = None
WORKERS = 8
RAM_LIMIT_GB = 24
GPU_INDEX = 0
...
FLOW_NAME = "flow_1.predict_cellpose_sam"
ORIGINAL_SCRIPT = PROJECT_ROOT / "scripts" / "02-inference" / "run_cellpose_sam.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Flow 1 wrapper for Cellpose SAM inference.")
...
def build_command(args: argparse.Namespace) -> list[str]:
    ...
    append_option(command, "--batch-size", args.batch_size)
...
def main() -> None:
    ...
    log(FLOW_NAME, f"workers={args.workers} ram_limit_gb={args.ram_limit_gb:g} batch_size={args.batch_size} gpu_index={args.gpu_index}")
